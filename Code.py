...
def find_latest_file(folder, prefix):
    files = [
        f for f in os.listdir(folder) if f.startswith(prefix) and f.endswith(".json")
    ]
    if not files:
...
def main():
    ...
    standardized_spec_file = os.path.join(
        os.path.dirname(__file__),
        "Data",
        "Standard_spec",
        f"standard_spec_{timestamp}.json",
    )
    ...
    generated_code_file = os.path.join(
        ...
        "Data",
        "Generated",
        f"generated_code_{timestamp}.json",
    )

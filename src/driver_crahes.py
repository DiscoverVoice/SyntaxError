...
def run_minimization(time_limit, memory_limit):
    if not os.path.exists("./output_dir/min_crashes"):
        os.mkdir("./output_dir/min_crashes")

    for file in tqdm(os.listdir(f"./output_dir/fuzzer01/crashes/")):
        if "id" in file:
            tmin_command = [
                "afl-tmin",
                "-i",
                f"./output_dir/fuzzer01/crashes/{file}",
                "-o",
                f"./output_dir/min_crashes/{file}",
                "-t",
                str(time_limit),
                "-m",
                str(memory_limit),
...
def main(args):
    try:
        ...
        if not os.path.exists("./profraw_crashes"):
            os.mkdir("./profraw_crashes")

        for testcase in os.listdir("./output_dir/min_crashes"):
            if "id" in testcase:
                run_command(
                    [
                        "bash",
                        "-c",
                        f"LLVM_PROFILE_FILE='./profraw_crashes/default_%p.profraw' ./temp < ./output_dir/min_crashes/{testcase} > /dev/null 2>&1",
                    ]
                ).wait()

        import glob

        profraw_files = glob.glob("./profraw_crashes/default_*.profraw")

        print("Running llvm-profdata merge...")
        profdata_merge_command = (
            ["llvm-profdata", "merge", "-sparse"]
            + profraw_files
            + ["-o", "./profraw_crashes/temp.profdata"]
        )
        ...
        llvm_cov_command = [
            ...
            "show",
            "./temp",
            "-instr-profile=./profraw_crashes/temp.profdata",
        ]

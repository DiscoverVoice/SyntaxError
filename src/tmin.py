...
def create_argument_parser():
    ...
    group.add_argument(
        "--auto", action="store_true", help="guess terminal condition by dry runs"
    )
    group.add_argument(
        "--crash", action="store_true", help="Crash (got signal or MSAN error)"
    )
    ...
    group.add_argument(
        ...
        metavar="N",
        type=int,
        help="try few times (default: %(default)s)",
        default=1,
    )
    group.add_argument(
        "--all",
...
def run_target_once(opts, data, filename=None):
    ...
    if g_execs == 1 and opts.auto:
        ...
        if p.returncode >= 0:
            ...
            if opts.returncode != 0:
                ...
                auto_found = True
        m = re.search(rb"ERROR: AddressSanitizer: (.+) on", stderr)
        if m:
            opts.stderr += [m.group()]
            m = re.search(rb"(READ|WRITE) of size ", stderr)
            if m:
                opts.stderr.append(m.group())
            m = re.search(rb"(SCARINESS: \d+ .*)", stderr)
            if m:
                opts.stderr.append(m.group())
    ...
    if opts.returncode is not None and opts.returncode != p.returncode:
        return False
    if opts.timeout is not None and opts.timeout != (
        opts.time_limit < (t1 - t0) * 1000
    ):
        return False
    ...
    while del_len >= 1 and in_len >= 1:
        ...
        while del_pos < in_len:
            ...
            if (
                not prev_del
                and tail_len
                and data[del_pos - del_len : del_pos]
                == data[del_pos : del_pos + del_len]
            ):
                del_pos += del_len
                continue

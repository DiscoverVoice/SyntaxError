...
def generate_strings(N, fuzz_out_dir, corpus_dir, fuzz_loc, san_fuzz_loc, cmp_fuzz_loc):
    ...
    used_flags = {
        flag: set()
        for flag in ["AFL_DISABLE_TRIM", "AFL_KEEP_TIMEOUTS", "AFL_EXPAND_HAVOC_NOW"]
    }
    used_args = {
        flag: set()
        for flag in ["-L 0", "-Z", "-P explore", "-P exploit", "-a binary", "-a ascii"]
    }
...
def main():
    ...
    parser.add_argument(
        "--fuzz-out", type=str, required=True, help="Fuzz output directory"
    )
    parser.add_argument("--corpus", type=str, required=True, help="Corpus directory")
    parser.add_argument(
        "--fuzz-loc", type=str, required=True, help="Location of the main fuzzer binary"
    )
    parser.add_argument(
        "--san-fuzz-loc",
        type=str,
        required=True,
        help="Location of the sanitizers-fuzzer binary",
    )
    parser.add_argument(
        "--cmp-fuzz-loc",
        type=str,
        required=True,
        help="Location of the cmplog-fuzzer binary",
    )

    args = parser.parse_args()

    result_strings = generate_strings(
        args.num_configs,
        args.fuzz_out,
        args.corpus,
        args.fuzz_loc,
